...
async function isWebsiteAlive(url) {
    try {
...
async function findByUrlPatterns(name) {
    const candidates = generateCandidates(name);
    for (let i = 0; i < Math.min(candidates.length, 15); i += 3) {
        const batch = candidates.slice(i, i + 3);
        const results = await Promise.all(batch.map(async (url) => (await isWebsiteAlive(url) ? url : null)));
...
async function searchDDG(name, city) {
    try {
        ...
        if (url && isValidBusinessUrl(url)) {
            logger_1.logger.debug('DDG found website', { name, url });
            return url;
        }
...
exports.enrichBusiness = enrichBusiness;
//# sourceMappingURL=webEnrichment.service.js.map